class NewtonInterpolation():
    """Реализация интерполяции с помощью полинома Ньютона."""
    ...
    def interpolate(self):
        """Производит интерполяцию полиномом Ньютона."""
        self.start_index = self._configurationSet()

        self._listDividedDiffInput()

        self.value_y = self._polynomialValue()
    ...
    def _polynomialValue(self) -> float:
        """Вычисляет полином n-ой степени для аргумента x."""
    ...
    def twoDerivativeThirdPolynomialDegree(self, argument_x: float) -> float:
        """Вычисляет 2-ую производную полинома 3-ей степени."""
